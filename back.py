...
class Cust_Reader:
    def __init__(self, connect_v,cust_req,nodescsv, interval = 5):
        ...
        self.table_shovel = pd.DataFrame.from_dict(self.sh.worksheet(\
            'Shovel').get_all_records())
        self.table_billing = pd.DataFrame.from_dict(self.sh.worksheet(\
            'Billing').get_all_records())
        self.sched = 0
        self.max_id_cust = 0
        ...
        if self.table_stockpile_ini.shape[0]>0:
            ...
            for st_ini in stocks_ini:
                data_sel = self.table_stockpile_ini[self.table_stockpile_ini['stock_name']\
                    == st_ini]
        ...
        self.image = 'Petegray.jpg'
        # position of each fixed location
        self.fixedloc = {row['fixed_name']:[row['fixed_x'],row['fixed_y']] \
            for row in self.fixedloc_ini}
        self.stock_id = {row['fixed_name']: [row['fixed_id'],row['stock_cost_ton']]\
            for row in self.fixedloc_ini if 'stock' in row['fixed_name']}
        # list of stocks
        self.table_fixedloc = self.info_fixed_loc()
        self.setstocks = list(self.table_fixedloc[self.table_fixedloc\
            ['fixed_type'] == 'stock']['fixed_name'])
    ...
    def get_brute_force(self):
        ...
        for x in permutation:
            ...
            delay_each = input_opt(assignment, self.shovelnode, decision_time, decision_time_sum,\
    ...
    def read_csv_2(self):
        ...
        **output:(0) new_out(df)             : plot direction of segments
            ...
            """         
        segment, points = filter_data(self.nodescsv,'PeteLien.png')
    ...
    def get_map(self):
        ...
        for index,  location in enumerate(self.fixedloc):
            ...
            ax.annotate(location,xy = (coordinate[0],coordinate[1]), xytext = (coordinate[0],coordinate[1]-25)\
                , c= color_f,fontsize =6)
    ...
    def get_data_animation(self):
        ...
        if self.requirement.shape[0]:
            ...
            for index_ind, datos in enumerate(self.to_w_m):
                ...
                time_to_load_sec = int(math.ceil(self.truck_capacity[index_ind]/self.loader_payload))\
                    *int(self.loader_cycletime)
                time_to_stock_sec = time_to_stock+time_to_load_sec     #use this for sec
                time_to_load = int(math.ceil(self.truck_capacity[index_ind]/self.loader_payload))\
                    *int(self.loader_cycletime/self.interpolator)
            ...
            return self.shape_matrixmom_sec(),self.new_out, self.c_order, self.fixedloc,\
                                self.loc_piles, delay_master, self.truck_capacity,\
                                    self.to_master_text,self.to_w, self.to_w_m,self.min_dectime,\
                                        self.interpolator
        else:
            return None
        ...
        for index, key in enumerate(self.dict_delay_sec):
            ...
            if add_db:
                ...
                list_bill.append([self.max_billing,customer,truck_db, self.truck_ton[truck_db],\
                    int(self.truck_ton[truck_db]*price), int(end_time_end+5), int(self.sched)])
